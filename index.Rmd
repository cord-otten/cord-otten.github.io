...
This is a prediction task [@rossi2014ms].
...
```{r scrape data from boxofficemojo.com, eval = FALSE}
...
map_dfr(seq(max(pages$group)), function(i) {
  pages %>%
    filter(group == i) %>%
    pull(id) %>%
    (function(x) {
...
d %>%
  ...
  select(
    title, release, opening, date, views
    ) -> d
...
```{r table with top movies, eval = TRUE}

da %>%
  arrange(desc(opening)) %>%
  mutate(top = seq(nrow(.))) %>%
  top_n(10, opening) %>%
  select(top, title, release, opening) %>%
  kable(
...
```{r plot bo distribution, eval = TRUE}
...
```{r plot clicks over time, eval = TRUE}
...
```{r available clicks prior to release, eval = TRUE}
...
```{r plot bo wiki correlation, eval = TRUE, fig.height = 10}
...
I highly recommend the book *Statistical Rethinking* by @mcelreath2018.  
...
$$opening \sim GammaPoisson(\lambda, \gamma)$$
$$log(\lambda) = a + \beta VIEWS$$
## Priors
