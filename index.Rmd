...
N <- d %>% select(title) %>% distinct() %>% nrow()

```


The sample consists of `r N`
titles.
...
d %>%
  ...
  geom_point() +
  geom_text(
    aes(label = ifelse(opening > 2e8, title, "")),
    hjust = -0.1, vjust = 0,
    alpha = .5) +
  scale_y_continuous(
...
d %>%
  ...
  top_n(10, opening) %>%
  kable(
    caption = str_c("Top 10 movies (sample size = ", N, ")"),
...
d %>%
  ...
  distinct() %>%
  arrange(desc(opening)) %>%
  mutate(rank = rank(desc(opening))) %>%
  ggplot(aes(x = rank, y = opening / 1e6)) +
  geom_col() +
  scale_y_continuous(
    name = "Box Office Opening Weekend [in million $]",
    labels = comma) +
...
```{r plot bo wiki correlation, eval = TRUE, echo = TRUE}
