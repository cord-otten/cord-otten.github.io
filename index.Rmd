...
references:
- id: rossiInvitedPaperEven2014
  type: article-journal
  title: >-
    Invited Paperâ€”Even the Rich Can Make Themselves Poor: A Critical Examination
    of IV Methods in Marketing Applications
  ...
  abstract: >-
    Marketing is a field that is rich in data. Our data is of high quality,
    often at a highly disaggregate level, and there is considerable variation in
    the key variables for which estimates of effects on outcomes such as sales
    and profits are desired. The recognition that, in some general sense,
    marketing variables are set by firms on the basis of information not always
    observable by the researcher has led to concerns regarding endogeneity and
    widespread pressure to implement instrumental variables methods in marketing
    problems. The instruments used in our empirical literature are rarely valid
    and the IV methods used can have poor sampling properties, including
    substantial finite sample bias and large sampling errors. Given the problems
    with IV methods, a convincing argument must be made that there is a first
    order endogeneity problem and that we have strong and valid instruments
    before these methods should be used. If strong and valid instruments are not
    available, then researchers need to look toward supplementing the
    information available to them. For example, if there are concerns about
    unobservable advertising or promotional variables, then the researcher is
    much better off measuring these variables rather than using instruments
    (such as lagged marketing variables) that are clearly invalid. Ultimately,
    only randomized variation in marketing variables (with proper implementation
    and large samples) can be argued to be a valid instrument without further
    assumptions.
  URL: 'http://pubsonline.informs.org/doi/abs/10.1287/mksc.2014.0860'
  ...
  accessed:
    - year: 2017
      month: 4
      day: 25
  title-short: Invited Paperâ€”Even the Rich Can Make Themselves Poor
...
digraph dag {
node [shape = box]
'Consumer Interest' -> 'Box Office'
'Consumer Interest' -> 'Wikipedia Views'
}")

```

This is a prediction task [@rossiInvitedPaperEven2014].
...
d %>%
  ...
  ggplot(aes(x = t, y = views / 1e6, group = title)) +
  geom_line(alpha = .1) +
...
$$opening \sim Poisson(\lambda)$$
$$log(\lambda) = a_{i}$$
$$a_{i} \sim Normal(\bar{a}, \sigma)$$
$$\bar{a} \sim Normal(16, 1)$$
...
da %>%
  ggplot(
    aes(x = opening)) +
  geom_density(aes(linetype = "data")) +
  stat_function(
    aes(linetype = "prior"),
    fun = function(x) {dlnorm(x, 18, 1.5)}) +
  scale_x_continuous(
    labels = comma) +
  scale_y_continuous(
    breaks = NULL,
...
```{stan output.var="m01", eval = FALSE}
...
model{
    ...
    sigma ~ exponential(1);
    aBar ~ normal(16 ,1.5);
    aTitle ~ normal(aBar ,sigma);
    for (i in 1:454) {
        lambda[i] = aTitle[title[i]];
        lambda[i] = exp(lambda[i]);
    }
    opening ~ poisson( lambda );
}
generated quantities{
    ...
    for (i in 1:454) log_lik[i] = poisson_lpmf(opening[i] | lambda[i]);
}
